...
export default function Stories() {
  ...
  const addDay = () => {
    ...
    addItem('stories', { id: uid('s_'), date, morningShot: false, closeUp: false, audience: false, note: '' }, { prepend: true })
  }
      ...
      {rows.length === 0 ? (
        ...
        <div className="card table-scroll">
          <table className="table sheet">
            ...
            <tbody>
              {rows.map((s) => {
                const d = growDay(germ, s.date)
                const done = doneCount(s)
                return (
                  <tr key={s.id}>
                    <td><input type="date" className="cell" value={s.date} onChange={(e) => upd(s.id, { date: e.target.value })} /></td>
